...
class ball(game_objects):
    ...
    def update(self):
        ...
        self.move_to(
            self.speed * math.cos(self.angle), -self.speed * math.sin(self.angle)
        )
...
class Game_Board(Canvas):  # construct a window
    ...
    cheatcode = (
        "LLRRLLRR"
        """Cheat Code -->   <Left><Left><Right><Right><Left><Left><Right><Right>"""
    )
    keyString = ""
    saveSequence = "SSSS"
    name = ""
    ...
    def __init__(self, root):
        ...
        self.ball = ball(self)
        self.pauseText = self.create_text(
            constants.width / 2,
            constants.height / 2,
            text="Paused\n\n\n\nPress <Return> to Resume\n\nPress <Down> for Leader Board",
            font=("Arial", 20),
            fill="#cccccc",
            justify="center",
            state="hidden",
        ...
        self.livesCounter = self.drawText(
            constants.width / 4, 10, "Lives: %s" % self.lives, 20
        )
        self.scoreCounter = self.drawText(
            constants.width * 3 / 4, 10, "Score: %s" % self.score, 20
        )
        self.Boss_text = self.create_text(
            constants.width / 2,
            constants.height / 2,
            text="Excel Loading...",
            font=("Arial", 25),
    ...
    def configure_paddle_ball(self):
        ...
        self.paddle.reset(
            (constants.width - self.paddle.width) / 2,
            constants.height - self.paddle.height,
            (constants.width + self.paddle.width) / 2,
            constants.height,
        )
        self.paddle.configure("#7f8c8d")
        self.ball.reset(
            constants.width / 2 - self.ball.radius,
            constants.height - self.paddle.height - 2 * self.ball.radius,
            constants.width / 2 + self.ball.radius,
            constants.height - self.paddle.height,
        )
    ...
    def reset(self):
        ...
        self.instruction = self.create_text(
            constants.width / 2,
            constants.height / 2,
            ...
            for i in range(constants.bricks_per_line * 2):
                ...
                self.bricks.append(
                    brick(
                        self,
                        amt * brick.width,
                        (lyr + 2) * brick.height + 25,
                        (amt + 1) * brick.width,
                        (lyr + 3) * brick.height + 25,
                        1,
                    )
                )
        if level == 2:
    ...
    def showText(self, text, hide=True, callback=None, speed=2500):
        ...
        self.textContainer = self.create_rectangle(
            0, 0, constants.width, constants.height, fill="#ffffff", width=0
        )
                ...
                if not self.key_input[3]:
                    if not self.key_input[2]:
                        if self.paddle.thrown and not (self.textStatus):
                            ...
                            while i < len(self.bricks):
                                if not self.check_collisions(self.ball, self.bricks[i]):
                                    if col == 1 or col == 3:
                                        self.ball.angle = (
                                            math.radians(180) - self.ball.angle
                                        )
                                    elif col == 2 or col == 4:
                                    ...
                                    if self.bricks[i].hits == 0:
                                        ...
                                        self.bricks[i].configure(
                                            brick.colours[self.bricks[i].hits]
                                        )
                                i += 1
                            self.won = len(self.bricks) == 0
                            if not (self.check_collisions(self.ball, self.paddle)):
                                self.ball.angle = math.radians(90) - math.atan(
                                    (
                                        self.ball.get_position()[0]
                                        + self.ball.radius
                                        - self.paddle.get_position()[0]
                                        - self.paddle.width / 2
                                    )
                                    / 70
                                )
                        ...
                        if self.key_input[0]:
                            ...
                            if (
                                not (self.paddle.thrown)
                                and self.paddle.get_position()[0] > 0
                            ):
                            ...
                            if (
                                not (self.paddle.thrown)
                                and self.paddle.get_position()[2] < constants.width
                            ):
                                self.ball.move_to(self.paddle.speed, 0)
                        self.cheat_code()
                        self.saving()
                        if not (self.textStatus):
                            if self.won:
                                ...
                                if self.level <= 2:
                                    self.showText(
                                        "You WIN!",
                                        callback=lambda: self.loading_bricks(
                                            self.level
                                        ),
                                    )
                                else:
                                ...
                                self.showText(
                                    "You LOSE!",
                                    callback=lambda: self.loading_bricks(self.level),
                                )
...
def Pressed(event):
    ...
    if event.keysym == "Left":
        ...
        board.paddle.thrown == False and not board.key_input[2] and event.keysym == "Up"
    ):  # Only Reload works before the game started.
        board.reload = True
    elif (
        board.paddle.thrown == False or board.key_input[2]
    ) and event.keysym == "Down":  # Leadership board works only before the game started and the game is paused.
        board.leader = True
    elif (
        event.keysym == "space" and board.paddle.thrown
    ):  # for saving.Only when gmae is not paused.
        board.keyString += "S"
    elif (
        event.keysym == "space" and board.textStatus == False and not board.key_input[3]
    ):  # to start a game.
        board.paddle.thrown = True
        board.delete(board.instruction)
    elif (
        event.keysym == "Return" and board.paddle.thrown
    ):  # To pause and resume a game.
    ...
    ):  # Boss key. paused and text will be showed for 7000 seconds. Have to resume by click return.
